...
import type { IParkingLocation } from '@/data/vehicle';

interface ParkingMapSectionProps {
  parkingLocation: IParkingLocation | null;
  onImageClick: (url: string) => void;
}

export default memo(function ParkingMapSection({ parkingLocation, onImageClick }: ParkingMapSectionProps) {
      ...
      <div className="space-y-4">
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/40 py-12">
          <MapPin className="size-10 text-muted-foreground/40 mb-3" />
           <p className="text-sm text-muted-foreground">No parking location assigned</p>
        </div>
      </div>
    );
  }

  const { name, diagramUrls } = parkingLocation;
  ...
  return (
    <div className="space-y-4">
      ...
      {diagramUrls.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/40 py-12">
          <MapPin className="size-10 text-muted-foreground/40 mb-3" />
           <p className="text-sm text-muted-foreground">No parking map available for this location</p>
        </div>
      ) : (
        <div
          className={
        ...
        >
          {diagramUrls.map((url, idx) => (
            ...
            >
              <Image
                src={url}
                alt={`${name || 'parking lot'} — parking map ${idx + 1}`}
